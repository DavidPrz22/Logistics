/*
  ...
  - You are about to drop the `fuenteTasa` table. If the table is not empty, all the data it contains will be lost.
  - You are about to drop the column `fuenteTasaId` on the `tasaCambio` table. All the data in the column will be lost.
  - Added the required column `registroTasasId` to the `tasaCambio` table without a default value. This is not possible if the table is not empty.

*/
-- DropIndex
DROP INDEX "fuenteTasa_nombre_key";

-- DropTable
PRAGMA foreign_keys=off;
DROP TABLE "fuenteTasa";
PRAGMA foreign_keys=on;

-- CreateTable
CREATE TABLE "registroTasas" (
...
CREATE TABLE "new_tasaCambio" (
    ...
    CONSTRAINT "tasaCambio_registroTasasId_fkey" FOREIGN KEY ("registroTasasId") REFERENCES "registroTasas" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
