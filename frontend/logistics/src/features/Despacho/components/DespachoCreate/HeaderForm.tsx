  ...
  return (
    <Card className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Field label="Cliente">
          <Combobox items={clientes.map((c) => ({ value: String(c.id), label: c.nombre, hint: c.direccion ?? undefined }))} value={cliente} onChange={onClienteChange} placeholder="Elegir cliente…" />
        </Field>
        <Field label="Chofer">
